from app.models import ma, Invoice
...
class InvoiceSchema(ma.SQLAlchemySchema):
    ...
    amount = fields.Decimal(required=True, strict=True, places=2)
    vat_amount = fields.Decimal(required=True, strict=True, places=2)
    ...
    def convert_types(self, data, many, **kwargs):
        if data:
            data['amount'] = float(data.get('amount')) if data.get('amount') >= 0 else None
            data['gross_amount'] = float(data.get('gross_amount')) if data.get('gross_amount') >= 0 else None
            data['vat_amount'] = float(data.get('vat_amount')) if data.get('vat_amount') >= 0 else None
        return data
