...
class InvoiceLineSchema(ma.SQLAlchemySchema):
    ...
    def check_line_quantity(self, data, **kwargs):
        ...
        if not check_quantity:
            raise ValidationError('Invalid type. Expected Number but got String.', 'quantity')
        return data
    ...
    def check_line_amount_type(self, data, **kwargs):
        ...
        if not check_amount_type:
            raise ValidationError('Invalid type. Expected Number but got String.', 'amount')
        return data
    ...
    def check_line_unit_price_type(self, data, **kwargs):
        ...
        if not check_unit_price_type:
            raise ValidationError('Invalid type. Expected Number but got String.', 'unit_price')
        return data

    @post_dump()
    def convert_decimal(self, data, many, **kwargs):
        data['quantity'] = float(data.get('quantity')) if data.get('quantity') >= 0 else None
