...
class Account(BaseModel):
    ...
    account_group_id = db.Column(
        db.Integer, db.ForeignKey("account_groups.id"), nullable=False
    )
    ...
    tax_rate: Mapped["TaxRate"] = relationship(
        "TaxRate", back_populates="accounts"
    )

    name = db.Column(db.String(100), primary_key=True, nullable=False)
    description = db.Column(
        db.String(100), server_default="", default="", nullable=True
    )
    number = db.Column(db.Integer, primary_key=True, nullable=False)
    currency_id = db.Column(
        db.String(10), server_default="DKK", default="DKK", nullable=False
    )
    ...
    bank_swift_number = db.Column(
        db.String(50), server_default="", default="", nullable=True
    )
    bank_iban_number = db.Column(
        db.String(50), server_default="", default="", nullable=True
    )

    is_bank_account = db.Column(
        db.Boolean,
        server_default=db.text("false"),
        default=False,
        nullable=False,
    )
    is_payment_enabled = db.Column(
        db.Boolean,
        server_default=db.text("false"),
        default=False,
        nullable=False,
    )
    is_deposit = db.Column(
        db.Boolean,
        server_default=db.text("false"),
        default=False,
        nullable=False,
