...
def seed():
    ...
    if not default_country:
        new_country = Country(
            currency_id="DKK", locale_id="da_DK", name="Danmark", icon="dk"
        )
        new_country.save()
    ...
    for org_json_path in organizations:
        with open(org_json_path, "r") as f:
            ...
            if not organization_obj:
                ...
                click.echo(
                    "already exists Organization: " + org_data.get("name")
                )
            ...
            click.echo(
                "\n-> creating products for Organization: "
                + org_data.get("name")
            )
            if org_products:
                for product_data in org_products:
                    ...
                    if not product_obj:
                        ...
                        product_obj.save()
                        click.echo(
                            "-- created product: " + product_data.get("name")
                        )
                    else:
                        click.echo(
                            "-- already exists: " + product_data.get("name")
                        )
            else:
            ...
            click.echo(
                "\n-> creating contacts for Organization: "
                + org_data.get("name")
            )
            if org_contacts:
                for contact_data in org_contacts:
                    ...
                    if not contact_obj:
                        ...
                        contact_obj.save()
                        click.echo(
                            "-- created contact: " + contact_data.get("name")
                        )
                    else:
                        click.echo(
                            "-- already exists: " + contact_data.get("name")
                        )

                    #####################
                    # contact invoices
                    click.echo(
                        "\n-- -> creating invoices for contact: "
            ...
            click.echo(
                "\n-> creating Tax-rates for Organization: "
                + org_data.get("name")
            )
            if tax_rates:
                for tax_rate_data in tax_rates:
                    ...
                    if not tax_rate_obj:
                        ...
                        tax_rate_obj.save()
                        click.echo(
                            "-- created Tax-rate " + tax_rate_data.get("name")
                        )
                    else:
                        click.echo(
                            "-- already exists: " + tax_rate_data.get("name")
                        )

            #################################
            # Organization built-in accounts
            click.echo(
                "\n-> creating Account types for Organization: "
                + org_data.get("name")
            )
            if account_types:
                for account_type_data in account_types:
                    # extract account_groups
                    account_type_groups = account_type_data.pop(
                        "account_groups"
                    )
                    ...
                    if not account_type_obj:
                        account_type_data["organization_id"] = (
                            organization_obj.id
                        )
                    ...
                    # account_groups
                    click.echo(
                        "\n-- -> creating account groups for account-type: "
                        + account_type_data.get("name")
                    )
                    ...
                    if account_type_groups:
                        for account_group_data in account_type_groups:
                            ...
                            if not account_group_obj:
                                account_group_data["organization_id"] = (
                                    organization_obj.id
                                )
                                account_group_data["account_type_id"] = (
                                    account_type_obj.id
                                )
                                account_group_obj = AccountGroup(
                                    **account_group_data
                                )
                                account_group_obj.save()
                                click.echo(
                                    "-- -- created Account group: "
                                    + account_group_data.get("name")
                                )
                            else:
                                click.echo(
                                    "-- -- already exists: "
                            ...
                            # accounts
                            click.echo(
                                "\n-- -- -> creating accounts for account-group: "
                                + account_group_data.get("name")
                            )
                                ...
                                for account_data in accounts:
                                    ...
                                    try:
                                        ...
                                        account_data["account_group_id"] = (
                                            account_group_obj.id
                                        )
                                        account_data["tax_rate_id"] = (
                                            tax_rate_id
                                        )
