...
def get_product(product_id):
    ...
    product = Product.query.filter_by(
        ...
        description=f"Product with id {product_id} for Organization id {organization_id} not found !"
    )
...
def delete_product(product_id):
    ...
    try:
        product.delete()
        return {
            "message": f"Product with id {product.id} is deleted from Organization id {product.organization_id}!!"
        }, 200
    except exc.IntegrityError:
        return {
            "error": "You can't delete this product, it is used in invoices"
        }, 202
...
def create_product():
    ...
    try:
        ...
        duplicate_check = Product.query.filter_by(
            organization_id=organization_id, name=product_data.get("name")
        ).first()
        if duplicate_check:
            raise Exception(
...
def update_product(product_id):
    ...
    product = Product.query.filter_by(
        organization_id=organization_id, id=product_id
    ).first_or_404(description=f"Product with id {product_id} not found!")
    ...
    try:
        ...
        duplicate_check = (
            ...
            .filter(not_(Product.id == product_id))
            .first()
        )

        if duplicate_check:
            return {
                "error": f"Product name {product_data.get('name')} already exists"
            }, 400
