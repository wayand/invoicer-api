...
def get_contact(organization_id, contact_id):
    contact = Contact.query.filter_by(
        ...
        description=f"Contact with id {contact_id} for Organization id {organization_id} not found !"
    )
...
def delete_contact(organization_id, contact_id):
    ...
    try:
        contact.delete()
        return {
            "message": f"Contact with id {contact.id} is deleted from Organization id {contact.organization_id}!!"
        }, 200
    except exc.IntegrityError:
        return {
            "error": "You can't delete this contact, it is used in invoices"
        }, 202
...
def create_contact(organization_id):
    ...
    try:
        ...
        duplicate_check = Contact.query.filter_by(
            organization_id=organization_id, name=contact_data.get("name")
        ).first()
        if duplicate_check:
            raise Exception(
...
def update_contact(organization_id, contact_id):
    contact = Contact.query.filter_by(
        organization_id=organization_id, id=contact_id
    ).first_or_404(description=f"Contact with id {contact_id} not found!")
    ...
    try:
        ...
        duplicate_check = (
            ...
            .filter(not_(Contact.id == contact_id))
            .first()
        )

        if duplicate_check:
            return {
                "error": f"Contact name {contact_data.get('name')} already exists"
            }, 400
        ...
        contact.registration_no = contact_data.get(
        ...
        contact.contactperson_name = contact_data.get(
            "contactperson_name", contact.contactperson_name
        )
        contact.contactperson_email = contact_data.get(
            "contactperson_email", contact.contactperson_email
        )
