...
from app.models import db, Organization, InvoiceSetting, Invoice, InvoiceLine, Product, invoice_schema_with_lines, invoice_setting, invoices_schema
...
def create_invoice(organization_id):
    ...
    try:
        ...
        duplicate_check = Invoice.query.filter_by(organization_id=organization_id, invoice_no=invoice_loaded.get('invoice_no')).first()
        if duplicate_check:
            raise Exception(f'Invoice_no ({duplicate_check.invoice_no}) already exists')
        lines = invoice_loaded.pop('lines')
        invoice = Invoice(**invoice_loaded, lines=[
...
def _invoice_no_exists(id, organization_id, invoice_no):
        return db.session.query(Invoice).filter(and_(
...
def update_invoice(organization_id, invoice_id):
    ...
    try:
        ...
        if _invoice_no_exists(invoice_id, organization_id, data.get('invoice_no')):
            return {'error': f'Invoice_no {data.get("invoice_no")} already exists'}, 400
        ...
        InvoiceLine.query.filter_by(invoice_id=invoice_id).\
            filter(InvoiceLine.id.not_in(to_be_deleted)).\
            delete(synchronize_session=False)

        for line in lines:
            if line.get('id'):
                invoiceline = InvoiceLine.query.filter_by(id=line.get('id'), invoice_id=invoice.id).first_or_404(description = f'InvoiceLine with id {line.get("id")} not found!')
                check_product = Product.query.filter_by(organization_id=organization_id, id=line.get('product_id')).first_or_404(description = f'Product with id {line.get("product_id")} not found!')
                invoiceline.product_id = line.get('product_id', invoiceline.product_id)
                invoiceline.quantity = line.get('quantity', invoiceline.quantity)
                invoiceline.amount = line.get('amount', invoiceline.amount)
                invoiceline.unit_price = line.get('unit_price', invoiceline.unit_price)
                invoiceline.description = line.get('description', invoiceline.description)
...
def delete_invoice(organization_id, invoice_id):
    ...
    try:
        invoice.delete()
        return { "message": f"invoice with id {invoice_id} and all its lines are deleted!!" }, 200
    except Exception as e:
        return {'error': str(e)}, 400