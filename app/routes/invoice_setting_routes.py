...
def update_invoice_setting():
    try:
        ...
        if not invoice_setting:
            ...
            invoice_setting.default_deposit_account_id = (
                invoice_setting_data.get(
                    "default_deposit_account_id",
                    invoice_setting.default_deposit_account_id,
                )
            )  # 5, # Bank, den primære bankkonto
            invoice_setting.template_id = invoice_setting_data.get(
                "template_id", invoice_setting.template_id
            )  # 1, # default template id
            invoice_setting.hide_product_numbers = invoice_setting_data.get(
                "hide_product_numbers", invoice_setting.hide_product_numbers
            )  # False, # Hide product number in invoice lines
            invoice_setting.lines_incl_vat = invoice_setting_data.get(
                "lines_incl_vat", invoice_setting.lines_incl_vat
            )  # False, # Invoice lines amount including vat
            invoice_setting.invoice_no_mode = invoice_setting_data.get(
                "invoice_no_mode", invoice_setting.invoice_no_mode
            )  #'Sequential', # Invoice numbering mode: Sequential OR Manual
            invoice_setting.next_invoice_no = invoice_setting_data.get(
                "next_invoice_no", invoice_setting.next_invoice_no
            )  # 1, # The next invoice number, if invoice-numbering: Sequential
            invoice_setting.default_reminder_fee = invoice_setting_data.get(
                "default_reminder_fee", invoice_setting.default_reminder_fee
            )  # 100
